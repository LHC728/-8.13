...
- **测试与回归**：requal 套件 **124/124 PASS** + 回归 **169/169 PASS**（`tests/test_h2_p3c_requal_v1.py` ACT-01..08 / DPKEY-01..05 / QUOTA-DP-01..03 / SAMPLE-01..04 等；证据 `05_结果/H2/requalification/2026-08-16_decision_semantics/tests_report.json`）。
- **成本重认证**：冻结网格 (4,8)/(8,6)/(8,8)（w_p≤90s；max M 再大 C_eval）三候选全可行 → 仍选 **(8,8)** → **冻结恢复 M\* = 8、C_eval\* = 8、W_cap\* = 4、P_cap\* = 4**（`cost_requalification_report.json`；实测 w_p_median (4,8)=3.40s / (8,6)=5.00s / (8,8)=6.60s）。
- **Pro-Max outcome-aware 审查（MMR V2.1.2 YELLOW 门）**：fresh `deepseek-pro-max/deepseek-v4-pro/max` 只读审查 → **PASS_WITH_CAVEAT**，`authority_conflict=false`，`required_actions`（R1 CURRENT_STATE/CHANGELOG 同步——本条目闭合；R2 重跑证据记录 B-1 口径——随重跑闭合）→ `required_actions_closed=true` → **P3-C 重跑 AUTHORIZED**（证据 `05_结果/governance/model_routing/v2_1_2/p3_requal_pro_max/`）。
- **阶段状态**：P3-C 重跑 = **待执行**（新 run root；deviation_count=0 → H2_DELETE → STOP Human Gate）；**cross-K transfer / h2_holdout / C25 保持 NOT AUTHORIZED 直至重跑完成**；Density / P1 / P2 / P3-A / P3-B / Q2 / G3 accepted 证据 = UNAFFECTED。
