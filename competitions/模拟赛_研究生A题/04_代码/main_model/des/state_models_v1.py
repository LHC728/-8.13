...
Implements the frozen explicit state model of G2-03-SPEC-V1.0.2 section 6
...
V1.0.2 deltas reflected here: DEVICE_STATE.d_state is materialized in the E2
substep (A/B/C all-PASS timestamp, before the E TASK_RELEASE; never at the E
ACTIVITY_START, never regenerated by an E retest); BAY_STATE.EMPTY is a legal
explicit state only for the batch_size==1 second bay (SEM-21/EMPTY), with
batch_size>=2 always preloading [1,2] (enforced by config validation).
...
# ---------------------------------------------------------------------------
# Frozen constants (G2-03-SPEC-V1.0.2)
# ---------------------------------------------------------------------------
...
class BayStatus(str, enum.Enum):
    ...
    # V1.0.2 (SEM-21/EMPTY final ruling): EMPTY is a legal explicit state only
    # when batch_size==1 (the second bay physically holds no device, e.g.
    # F1/F3/F5/F6/F11/F12). batch_size>=2 always preloads [1,2] at t=0
    # (bay1=device1, bay2=device2), so EMPTY can never be used to bypass the
    # first-two preloaded devices; that bypass is additionally blocked by the
    # SEM-21 config validation (DesPreloadRuleError) in deterministic_des_v1.
    EMPTY = "EMPTY"
