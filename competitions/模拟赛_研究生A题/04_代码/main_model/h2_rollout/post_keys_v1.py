...
def rollout_post_keys(master_seed_h2: int, replicate_id: int, dp: int, m: int,
                      ...
                      salt: str = "q3h2-bootstrap-v1") -> RolloutPostKeys:
    ...
    same m-world draws (frozen CRN rule, SPEC 6.2).  ``salt`` selects the
    rollout salt (normal ROLLOUT_SALT or the frozen ALT salt for stability
    (b)).
    """
    from main_model.h2.rollout_seed_v1 import rollout_seed as _rs
    seed = _rs(master_seed_h2, replicate_id, dp, m, salt=salt)
...
# ---------------------------------------------------------------------------
# Physical-world post keys (h2_tuning / h2_holdout batch generation)
# ---------------------------------------------------------------------------
# The batch (physical) worlds under h2_tuning / h2_holdout are generated by
# the continuation engine consuming the *post* streams under those
# namespaces (the accepted legacy engine structurally rejects H2
# namespaces; the H2 batch driver uses the post-key provider below).
# Physical keys are distinct from h2_rollout rollout keys (C16 disjoint).


def physical_post_provider(namespace: str, master_seed: int,
                           replicate_id: int,
                           batch_size: int,
                           resources: tuple[str, ...]
                           ) -> re1.PostKeyProvider:
    """Build a PostKeyProvider for the PHYSICAL batch world under an H2
    namespace (h2_tuning / h2_holdout).  Keys follow the frozen canonical
    slot pattern (SPEC 6.2) with the given namespace/seed/rep; the engine
    consumes U_X_post (per subsystem for not-yet-entered priors), U_D_post
    (junction), U_Y_post (valid completions) and U_L_post (per generation).
    """
    from main_model.h2_rollout import rollout_engine_v1 as re1

    def _u_x(device: int) -> Fraction:
        return ks.u_x_post(namespace, replicate_id, device,
                           ABC_CATEGORICAL_SUBSYSTEM, master_seed)

    def _u_x_sub(device: int, subsystem: str) -> Fraction:
        return ks.u_x_post(namespace, replicate_id, device, subsystem,
                           master_seed)

    def _u_d(device: int) -> Fraction:
        return ks.u_d_post(namespace, replicate_id, device, master_seed)

    def _u_y(device: int, process: str, attempt: int) -> Fraction:
        return ks.u_y_post(namespace, replicate_id, device, process, attempt,
                           master_seed)

    def _u_l(resource: str, generation: int) -> Fraction:
        return ks.u_l_post(namespace, replicate_id, resource, generation,
                           master_seed)

    devices = tuple(range(1, batch_size + 1))
    u_x = {d: _u_x(d) for d in devices}
    u_x_sub = {d: {s: _u_x_sub(d, s) for s in ks.SUBSYSTEMS} for d in devices}
    u_d = {d: _u_d(d) for d in devices}
    u_l = {r: _u_l(r, 1) for r in resources}
    return re1.PostKeyProvider(
        u_x_by_device=u_x, u_x_subsystem_lookup=_u_x_sub,
        u_d_by_device=u_d, u_l_by_resource=u_l,
        u_y_lookup=_u_y, u_l_lookup=_u_l)
