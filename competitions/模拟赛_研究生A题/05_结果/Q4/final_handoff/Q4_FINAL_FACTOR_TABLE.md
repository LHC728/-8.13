...
- Q-B 解释为 **incoming defect pressure（上游来料质量问题率）**，可用于描述来料质量变化风险，不得写成「调 q 就能降低 T」。
- e 解释为 **measurement-error environment（测量误差环境）**，可作为质量/测试管理风险因素；**不得把 alpha/beta 拆成独立控制变量**。
