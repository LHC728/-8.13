...
REAL_UPSTREAM_ROOT = os.path.join(PROJECT_ROOT, "05_结果", "G2",
                                  "run_20260814T130221390333Z_8babb503")

FIXED_RUN_ID = "20260814T130221390333Z_abcd1234"
UPSTREAM_RUN_ID = "20260814T130221390333Z_8babb503"
...
OLD_SPEC_VERSION = "G2-02-SPEC-V1.0.%d" % 4
...
class SpecBindingTests(unittest.TestCase):
    def test_spec_version_and_hashes_match_v105(self):
        """V1.0.5 spec/schema/fixture hashes correct => PASS (V1.0.5 =
        provenance-clean upstream rebind only)."""
        with open(REAL_TASK_PACKAGE, encoding="utf-8") as f:
            tp_text = f.read()
        self.assertEqual(runner.SPEC_VERSION, "G2-02-SPEC-V1.0.5")
        self.assertEqual(runner.extract_task_package_version(tp_text), "G2-02-SPEC-V1.0.5")
        ...
        self.assertEqual(fs["upstream_chain_response"],
                         "49bad533cdacd2309c697f52636ebca8a895795c6247aea65e711911bcddae76")
        self.assertEqual(fs["upstream_file_hashes"],
                         "2a1e30fb116b47e01d01051435ea85d25ee1484d7793db038c65193de602b8b6")
    ...
    def test_check_spec_bindings_pure(self):
        ...
        self.assertEqual(
            runner.check_spec_bindings('task_package_version: "G2-02-SPEC-V1.0.5"\n', good), [])
        bad = dict(good)
        bad["schema"] = "9" * 64
        errs = runner.check_spec_bindings('task_package_version: "G2-02-SPEC-V1.0.5"\n', bad)
        ...
        self.assertTrue(any("task_package_version" in e for e in errs3),
                        "V1.0.3 must NOT be an active binding in V1.0.5")
    ...
    def test_wrong_spec_version_fails(self):
        """A non-V1.0.5 task package is refused (FAIL), never auto-updated."""
        code, manifest, has_commands = self._dry_run_with_task_package(
            [("G2-02-SPEC-V1.0.5", OLD_SPEC_VERSION)])
        self.assertEqual(code, 1)
        self.assertEqual(manifest["overall_status"], "FAIL")
