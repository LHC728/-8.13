...
- **Q3-H2-P3-C 决策语义重新认证（REQUALIFICATION IN PROGRESS，MMR V2.1.2 YELLOW；Human Gate BLOCKER A–D）= 修复完成 + Pro-Max PASS_WITH_CAVEAT（required_actions_closed=true、authority_conflict=false）→ P3-C 重跑已授权**：旧 P3-C root `run_20260816T101049163825Z_791506d8` = **INVALIDATED_FOR_DECISION_SEMANTICS**（n/agreement/deviation 数字保留不改；不再作为 H2 DELETE 决策证据；`INVALIDATION_STATUS.json` + `INVALIDATION_REPORT.md` 叠加记录）；P3-A 修复（pre-action 决策边界 + dispatch idle/available guard）、P3-B 修复（candidate first-action 在冻结决策上下文上严格生效、fail-closed；per-batch evaluated dp 语义；B-1 样本 = production-contract quota 实际评估点；checker rollout-kernel requalified）；测试 124/124 + 回归 169/169 PASS；成本重认证（冻结网格 (4,8)/(8,6)/(8,8)，w_p≤90s，max M 再大 C_eval）→ 仍选 **(8,8)** → **冻结恢复 M\* = 8、C_eval\* = 8、W_cap\* = 4、P_cap\* = 4**；证据 `05_结果/H2/requalification/2026-08-16_decision_semantics/`（tests_report / cost_requalification_report / first_action / dp_key / quota_dp / sample_topup / checker_requal）+ `05_结果/governance/model_routing/v2_1_2/p3_requal_pro_max/`（PASS_WITH_CAVEAT；R1 已闭合；R2 随重跑证据闭合）；**P3-C 重跑 = 待执行（新 run root；deviation_count=0 → H2_DELETE → STOP Human Gate）；cross-K transfer / h2_holdout / C25 保持 NOT AUTHORIZED 直至重跑完成**。
