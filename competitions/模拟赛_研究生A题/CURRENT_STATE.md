...
- **Q3-H2-P3 第二次语义重新认证（REQUALIFICATION IN PROGRESS，Human Gate：e4fd435 的 H2 DELETE 暂不接受）= 修复完成 + Pro-Max 第二轮 PASS_WITH_CAVEAT（authority_conflict=false、A–H 八问全 YES）→ P3-C 重跑已授权**：撤销第一轮「quota-evaluated = B-1 population」解释（登记 **HG-Q3-H2-DP-DIAG-01** addendum + **MODEL_ROUTING_MISS-001**）；最新 root `run_20260816T150705443207Z_4f60db0d` = **INVALIDATED_FOR_FROZEN_SAMPLE_AND_ROLLOUT_WORLD_SEMANTICS**（数字不改；旧 `791506d8` 保持原失效）；**B-1 恢复为 ALL H2-ELIGIBLE**（不受 C_eval\* 约束；dp_diag = 每批 (time, canonical resource order) 0-based；B1-OFFLINE-01/02/03 证明）；**posterior world_m**：evaluate_decision_point 逐 m 由 h2_rollout keys 重建 world_m/provider_m（同 m 跨动作共享；caller fixed world 移除；physical h2_tuning hidden world 不再进入 policy rollout）；**AGE-LEGAL-01/02/03**（a+d>240 无点；==240 无 optional PM）；**PENDING-01..04**（同 timestamp failure/illegal-240/mandatory 在 pre-action 状态可见，不读 hidden lifetime）；测试 **145/145 requal + 169/169 回归 PASS**；成本重认证（c_r 含 world_m 重建）→ 仍选 **(8,8)** → **冻结恢复 M\* = 8、C_eval\* = 8、W_cap\* = 4、P_cap\* = 4**；证据 `05_结果/H2/requalification/2026-08-16_decision_semantics/`（SECOND 系列）+ `05_结果/governance/model_routing/v2_1_2/p3_requal_pro_max/pro_max_review_second.txt`；**P3-C 重跑（第二次重认证语义：ALL-eligible B-1 + posterior world_m + AGE-LEGAL/PENDING/bay 修复）= COMPLETED → H2_NO_EFFECTIVE_DEVIATION → H2 = DELETE（冻结硬门；AUTOPILOT STOP）；cross-K transfer / h2_holdout / C25 保持 NOT AUTHORIZED 直至 Human Gate final Q3 review**。
