...
- **Q3-H2-P3-B（完整 rollout 执行核 + (M*,C_eval*) 成本冻结）= COMPLETED / AWAITING HUMAN GATE**：fresh continuation rollout engine（首动作后 H1 回退至吸收；future D 在合法联接点按 prior q_D 物化一次；U_Y_post 仅有效完成消费；新代 U_L_post）；H1 fallback parity 6 场景与 accepted H1 engine 核心事件 + T_end 逐场景一致（同一 u 流重放）；Q_hat_M + 配对 D_m/SE_M（纯计算，不据 Q 调参）；online quota selector（C_eval 6/8：W=⌈C/2⌉、P=⌊C/2⌋；wait 前 W_cap；PM 桶 B1/B2/B3 + P_cap=4 时 1 extra slot；在线因果、无未来查看/无回溯）；C_rollout=200 硬上限（96/144/192 ≤ 200）；独立 checker 11/11 + tests 17/17 + regressions 28/36/38/23/44；**成本校准（h2_tuning rep 0..4、K=10.5、engine-only base_r + single-rollout c_r）→ 三候选全可行，按冻结规则（w_p≤90s、max M、M 同取大 C_eval）选 (M*,C_eval*)=(8,8)、W_cap*=4、P_cap*=4、w_p=6.26s（worst 6.49s）**；C23 rollout path = PASS（full final policy C23 PENDING）；证据 `05_结果/H2/tuning/cost_calibration/run_<UTC>_<8hex>/`。**action stability / cross-K transfer / h2_holdout / C25 仍 NOT AUTHORIZED**。
