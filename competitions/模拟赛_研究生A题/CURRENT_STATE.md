...
- **Q3-H2-P3-A（决策点/动作/rollout/continuation 基础）= COMPLETED / AWAITING HUMAN GATE P3-A REVIEW**：决策点两类（dispatch / maintenance）+ canonical A/B/C/E ordering + dp 0-based 纯函数；五动作续演语义（START_HEAD / H1_NOOP / WAIT_EVENT STRICT|BOUNDARY / PM_WITH_HEAD / PM_IDLE；mandatory/exact_240 永非 policy choice）；rollout_seed(dp,m) 公式 + CRN（seed 不含 action/policy/run_id）；continuation world 重建（仅 ObservableState + PosteriorState + rollout post keys，严禁 deepcopy live world / 读隐藏字段）；C23 same-observed-history end-to-end mechanics；独立 checker 9/9 PASS（非空 toy：dispatch STRICT WAIT + maintenance 120h age 链）+ tests 25/25 + regressions 28/36/38/23/44；证据 `05_结果/H2/p3/mechanics/run_20260816T060111865845Z_d5eafe0e`（历史 superseded run `...055931412559Z_a5f9bd4e` = SUPERSEDED_FOR_CHECKER_CLEANUP，PRIMARY mechanics 有效）。**C23 = END-TO-END MECHANICS PASS；full production C23 PENDING UNTIL FINAL POLICY CONFIG**。
