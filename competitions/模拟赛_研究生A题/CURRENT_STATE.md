...
- **当前包（Human Gate 一体化授权）**：A. **Q4 Phase 1B q/e SCREENING**（逻辑域 Q4_SCREENING、物理 q4_screening、seed 7、**新 rep 20..39**；q 因素 = GLOBAL PROPORTIONAL DEFECT-PRESSURE SCALE（q_A/B/C/D 同乘 scale∈{0.8,1.2}）：Q-A = CALIBRATION_INPUT（重标定 A/B/C alpha,beta→q_E 传播→重标定 E；e 保持 baseline）、Q-B = INCOMING_DEFECT_PRESSURE（kernel 冻结 baseline，只改真实问题率）；e 因素 = E-ERROR_ENVIRONMENT（e_A/B/C/E 同 scale，feasibility ladder ±20%→±10%→±5%，e≤2min(q,1-q) 全链不可 clip，只按 deterministic feasibility 选级））；B. **Q4 FORMAL EVALUATION**（逻辑域 Q4_EVALUATION、物理 q4_evaluation、seed 7；初始 R=64（**rep 100..163**）；**target 95% CI half-width = 2.0 h**；two-stage：任一正式 scenario half-width>2.0h → 所有 scenarios 统一扩 R=128（追加 rep 164..227）；禁 factor-specific optional stopping；正式评价含全部预注册主 scenarios（baseline + F2 + F3×3 + F4×8 + F5 + Q-A×2 + Q-B×2 + E×2）+ interaction 2 cells（INT_F2_05_E_M10 / INT_F2_05_E_P10）；F1 K = REUSED_ACCEPTED_FORMAL_EVIDENCE（Q3 accepted，不与 Q4 混算 CI）；formal factor rank = `factor_score = max(|standardized_effect|)` per factor（冻结规则））；C. **Top-2 interaction（F2×F4-E）**（contrasts I_minus/I_plus，replicate-level paired 95% CI；CI 含 0 → 只说「未观察到明确 interaction」；排除 0 → 「模型内非加性交互」，禁现实因果；interaction CI 亦适用 2h half-width 规则）。**本包硬约束：Pro-Max calls = 0**（高峰价格区；final 论文语义审查留待 Human Gate 后低价时段另行执行）。**本包输出 FINAL STATUS = Q4_NUMERIC_RESULTS_READY_FOR_HUMAN_GATE 或 BLOCKED；完成后 STOP（不自动进入 G7 final paper freeze、不写 final paper claim、不改 Q3）。**
